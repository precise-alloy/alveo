---
description: 'Use when modifying frontend logic, alveo build tooling, Vitest coverage settings, or frontend CI workflows in PreciseAlloy.Frontend. Enforces the highest possible coverage, matching tests for every logic change, and bun run test:ci validation.'
name: 'Frontend Test Coverage Policy'
applyTo: 'src/**, alveo/**, package.json, vitest.config.ts, docs/test-coverage.md, .github/workflows/frontend-*.yml'
---
...
- Prefer extracting a testable core helper from side-effect-heavy `alveo` modules instead of accepting untested logic.
- Always cover newly introduced logic as much as possible. Any new `src/**` or `alveo/**` module that contains testable logic (pure helpers, regex/string transforms, dependency-injected core) MUST be added to the `coverage.include` list in `vitest.config.ts` in the same change, with tests that meet the enforced thresholds. Do not rely on the include list silently hiding a new file from the coverage report.
