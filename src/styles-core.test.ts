...
describe('alveo/styles-core.ts', () => {
  ...
  it('injects abstracts, functions, and mixins into component scss by default', () => {
    ...
    const result = prepareCssFileContent(
      { srcFile: 'src/organisms/hero/index.scss', projectRoot: '.' },
      {
  ...
  it('supports disabling abstracts, functions, or mixins when building content', () => {
    ...
    expect(
      prepareCssFileContent(
        { srcFile: 'src/assets/styles/01-mixins/_buttons.scss', projectRoot: '.', includeMixins: false },
        {
  ...
  it('converts in-workspace source paths to project-root-relative paths', () => {
    expect(getRelativeSourceMapPath('/workspace/PreciseAlloy.Frontend/alveo/styles/root.scss', '/workspace/PreciseAlloy.Frontend')).toBe(
      'alveo/styles/root.scss'
    );
  ...
  it('normalizes absolute non-url source paths to project-root-relative paths', () => {
    const source = '/workspace/PreciseAlloy.Frontend/alveo/styles/root.scss';
    ...
    expect(result.sources).toEqual(['alveo/styles/root.scss']);
    expect(consumer.originalPositionFor({ line: 1, column: 0 }).source).toBe('alveo/styles/root.scss');
    expect(consumer.sourceContentFor('alveo/styles/root.scss', true)).toBe('.root { color: red; }');
  });
  ...
  it('falls back to the raw source name when resolveSourceMapPath returns undefined (data: url)', () => {
    const dataSource = 'data:;charset=utf-8,%24c%3A%20red%3B';
    const generator = new SourceMapGenerator({ file: 'hero.css' });

    generator.setSourceContent(dataSource, '$c: red;');
    generator.addMapping({
      generated: { line: 1, column: 0 },
      original: { line: 1, column: 0 },
      source: dataSource,
    ...
    // The data: source cannot be resolved to a file path, so the original
    // source name should be preserved as-is (line 246 fallback).
    expect(result.sources).toContain(dataSource);
    expect(consumer.sourceContentFor(dataSource, true)).toBe('$c: red;');
    expect(consumer.originalPositionFor({ line: 1, column: 0 }).source).toBe(dataSource);
  });

  it('falls back to mapping.source when normalizedSources does not contain the mapping source', () => {
    // Construct a raw source map where a mapping references a source that
    // is in the "sources" array but was added AFTER the initial forEach,
    // so normalizedSources.get() returns undefined. We achieve this by
    // adding a source only to the raw JSON (not through the generator API
    // that would populate sourcesContent), and creating a VLQ segment that
    // references this second source index.
    //
    // VLQ segment "CAAA" at source index 1 means: column 0 relative,
    // source +1, line 0 relative, column 0 relative.
    const rawMap = {
      version: 3 as const,
      file: 'out.css',
      sources: ['file:///D:/repo/src/known.scss', 'phantom.scss'],
      sourcesContent: ['.known { color: red; }', null],
      names: [] as string[],
      mappings: 'AAAA;CCAA',
    };
    ...
    const result = stripInjectedPreludeFromSourceMap(
      ...
      {
        existsSync: vi.fn().mockReturnValue(true) as never,
        readFileSync: vi.fn().mockReturnValue('.known { color: red; }') as never,
      },
      { projectRoot: 'D:/repo' }
    );

    // The phantom source was not in normalizedSources (it has no
    // sourcesContent entry and is handled by the fallback path).
    // The result should still contain mappings without throwing.
