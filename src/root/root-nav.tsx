...
export default function RootNav({ routes: routesProp }: Props) {
  ...
  return (
    <>
      ...
      <a ref={buttonRef} className="alveo-o-root__button-close" onClick={handleClick}>
        <button aria-label={show ? 'Close' : 'Pages'} className="alveo-o-root__control-button alveo-o-root__button-close">
          <svg className="alveo-o-root__control-svg">
            <use xlinkHref={viteAbsoluteUrl('/assets/images/root.svg#' + (show ? 'close' : 'list'))} />
          </svg>
        </button>
      </a>

      <p className="alveo-o-root__title">{activeItem?.name}</p>

      <div ref={navItemRef} className={`alveo-o-root__nav-items ${show ? 'show' : ''}`}>
        <div className="alveo-o-root__search">
          <input
            ref={inputRef}
        ...
        <div className="alveo-o-root__items">
          {textSearch.trim() && (
            <div className="alveo-o-root__search-matches">
              {routesSearch.length
                ? routesSearch
                    ...
                    .map((item, index) =>
                      item.path ? (
                        ...
                        <details
                          key={index}
                          className="alveo-o-root__nav-item-collection"
                          open={
                ...
                : textSearch.trim() && <div className="alveo-o-root__nav-message ">No pattern matches</div>}
            </div>
          )}

          <div className={`alveo-o-root__search-not-matches ${textSearch.trim() ? 'blur' : ''}`}>
            {routes
              .filter((r) => r.path != '/' && r.path != '/index')
              .sort((a, b) => (a.name < b.name ? -1 : a.name > b.name ? 1 : 0))
              .map((item, index) =>
